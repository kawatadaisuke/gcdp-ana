...
subroutine output(step,ngp,flaggp,flagrlog,ng1,ng2,ng3,ng3i,ran1,ran2,ran3)
      use gcdp_const
      use gcdp_system
      use grid_particle
      ...
      integer,intent(in) :: step,flaggp,flagrlog,ng1,ng2,ng3,ng3i,ngp
...
! for work
      ...
      double precision rp,frp,lnri,lnro,dlnr,phip,ri,ro,dr
...
! for grid output
      ...
      if(myrank.eq.0) then
        if(flaggp.eq.0) then
          write(fileo,'(a15,i6.6,a4)') 'output/epotgrid',step,'.dat'
        else
          write(fileo,'(a17,i6.6,a4)') 'output/epotsqgrid',step,'.dat'
        endif
...
! for ascii output
        if(flaggp.eq.0) then
          write(fileo,'(a15,i6.6,a4)') 'output/epotgrid',step,'.asc'
        else
          write(fileo,'(a17,i6.6,a4)') 'output/epotsqgrid',step,'.asc'
        endif
        write(6,*) 'output file name=',fileo
        open(60,file=fileo,status='unknown')
        if(flaggp.eq.0) then
...
! *** write the data
          ...
          do i=0,npj-1
            ...
            if(rp.gt.0.0d0) then
! angle from -x,y=0, clockwise, y<0, negative phi
              phip=acos(tdvr(i)/rp)
              phip=M_PI-phip
